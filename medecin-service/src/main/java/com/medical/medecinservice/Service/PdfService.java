...
public class PdfService
{
    ...
    public void generatePdfFileFrofil(Med med, MultipartFile image) throws IOException {
        String modelUrl="medecin-service/src/main/resources/PdfProfil/Model.pdf";
        String destUrl="medecin-service/src/main/resources/PdfProfil/"+med.getMatricule()+".pdf";
        Path modelPath= Paths.get(modelUrl);
        ...
        insertPersonnalInfo(med,form);
        insertProfilImage(med,image,form);
        try {
            qrCodeService.generateQRCode(med.getMatricule(),300,300);
    ...
    private  void insertPersonnalInfo(Med med,PdfAcroForm form)
    {
        form.getField("mat").setValue(med.getMatricule());
        form.getField("prenom").setValue(med.getPrenom());
        form.getField("nom").setValue(med.getNom());
        form.getField("email").setValue(med.getEmail());
        form.getField("tel").setValue(med.getTel());
        form.getField("role").setValue("MEDECIN");
    }

    private void insertQRCode(PdfAcroForm form)
    {
        String qrCodeUrl= "medecin-service/src/main/resources/ImageProfil/qrCode.png";
    ...
    private void insertProfilImage(Med med, MultipartFile image, PdfAcroForm form)
            throws FileNotFoundException, MalformedURLException {
        //INSERTION DE L'IMAGE DANS LE DOSSIER 'ImageProfil'
        Path ImageProfil = Paths.get("medecin-service/src/main/resources/ImageProfil/");
        if (!Files.exists(ImageProfil))
        {
            throw new FileNotFoundException("Dossier inexistente");
        }

        Path dest=ImageProfil.resolve(med.getMatricule()+".png").normalize();
        try
        {
        ...
        String imageUrl="medecin-service/src/main/resources/ImageProfil/"+med.getMatricule()+".png";
